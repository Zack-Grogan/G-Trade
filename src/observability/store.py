...
class ObservabilityStore:
    ...
    def record_completed_trade(
        ...
        *,
        ...
        try:
            ...
            account_context = self._extract_account_context(payload)
            trade_run_id = str(run_id or payload.get("run_id") or self._run_id)
            record = {
                "run_id": trade_run_id,
                "inserted_at": self._serialize_datetime(datetime.now(UTC)),
            ...
            with self._lock:
                assert self._conn is not None
                if not self._is_authoritative_completed_trade_locked(trade_run_id):
                    logger.debug(
                        "Skipping completed trade for non-authoritative run_id=%s data_mode=%s",
                        trade_run_id,
                        self._run_data_mode_locked(trade_run_id),
                    )
                    return False
    ...
    def backfill_completed_trades_from_events(self, *, run_id: Optional[str] = None) -> dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                ...
                for row in rows:
                    checked += 1
                    if not self._is_authoritative_completed_trade_locked(str(row["run_id"])):
                        continue
                    payload = json.loads(row["payload_json"] or "{}")
                    account_context = self._extract_account_context(payload)
        ...
        while self._running or not self._queue.empty() or pending:
            ...
            if pending and (len(pending) >= batch_size or (time.time() - last_flush) >= flush_interval or not self._running):
                try:
                    ...
                    backoff_seconds = min(5.0, 0.25 * (2 ** min(self._flush_failure_count - 1, 4)))
                    logger.exception(
                        "Failed to flush observability events (attempt=%s, retry_in=%.2fs)",
    ...
    def _commit_locked(self) -> None:
        assert self._conn is not None
        if not self._batch_write_active:
            self._conn.commit()
    ...
    def _run_data_mode_locked(self, run_id: str) -> Optional[str]:
        assert self._conn is not None
        row = self._conn.execute(
            """
            SELECT data_mode
        ...
        if row is None:
            return None
        value = str(row["data_mode"] or "").strip().lower()
        return value or None

    def _is_authoritative_completed_trade_locked(self, run_id: str) -> bool:
        data_mode = self._run_data_mode_locked(run_id)
        return data_mode in {None, "live"}

    def _canonicalize_datetime_value(self, value: datetime | str | None) -> Optional[str]:
        if value in {None, ""}:
            return None
        if isinstance(value, datetime):
            return self._serialize_datetime(value)
        ...
        if parsed is None:
            return str(value)
        return self._serialize_datetime(parsed)
