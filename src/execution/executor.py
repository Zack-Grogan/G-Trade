...
class OrderExecutor:
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        if not cancelled:
            self._record_event(
                ...
                payload={"mock_mode": False, "error": last_error},
    ...
    def reconcile_broker_open_orders(
        self,
        symbol: str,
        broker_orders: Optional[List[Dict[str, Any]]],
        *,
        event_time: Optional[datetime] = None,
        reason: str = "broker_order_reconciliation",
        broker_position: int = 0,
    ) -> Dict[str, Any]:
        """Align local entry orders with broker open-order truth."""
        ...
        for order_id, order in list(
            ...
            order.updated_time = event_time
            self._pending_orders.pop(order_id, None)
            cleared_order_ids.append(order_id)
            self._record_event(
                event_type="broker_entry_order_cleared",
                payload={
                    "status": OrderStatus.CANCELLED.value,
                    "reason": reason,
                    "broker_order_ids": sorted(broker_order_map.keys()),
                    "is_protective": order.is_protective,
                    "role": order.role,
                    "lifecycle_state": self._lifecycle_state.value,
                },
        ...
        for order_id, broker_order in broker_order_map.items():
            ...
            adopted = Order(
                ...
                activation_time=event_time,
                is_protective=False,
                role="entry",
            )
            self._pending_orders[order_id] = adopted
            adopted_order_ids.append(order_id)
            self._record_event(
                event_type="broker_open_order_adopted",
                payload={
                    "quantity": quantity,
                    "side": side,
                    "order_type": order_type,
                    "limit_price": adopted.limit_price,
                    "stop_price": adopted.stop_price,
                    "reason": reason,
                    "is_protective": False,
                    "role": "entry",
                    "lifecycle_state": ExecutionState.WORKING.value,
                },
                event_time=event_time,
                symbol=symbol,
                action="broker_reconcile",
        ...
        if cleared_order_ids or adopted_order_ids:
            self._record_event(
                event_type="broker_open_order_reconciled",
                payload={
                    "reason": reason,
                    "cleared_order_ids": cleared_order_ids,
    ...
    def get_watchdog_snapshot(self, symbol: str = "ES") -> dict[str, Any]:
        ...
        return {
            ...
            "active_entry_order_ids": list(active_entry_orders.keys()),
