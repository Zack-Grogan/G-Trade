...
def atr(
    high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14, use_wilder: bool = True
) -> pd.Series:
    """
...
def atr_current(high: float, low: float, close: float, prev_close: Optional[float] = None) -> float:
    ...
    tr = max(high - low, abs(high - (prev_close or close)), abs(low - (prev_close or close)))
    return tr
