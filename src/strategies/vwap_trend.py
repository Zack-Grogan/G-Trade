...
class VWAPTrendStrategy(BaseStrategy):
    ...
    def compute_signal(
        self, df: pd.DataFrame, position: int, zone_info: Any = None
        ...
        self._atr_value = atr(
            df["high"], df["low"], df["close"], self.strategy_config.atr_length
        ).iloc[-1]

        self._vwap_value = vwap(
            df["high"], df["low"], df["close"], df["volume"], self.strategy_config.vwap_source
        ).iloc[-1]
        ...
        # Exit logic (in position)
            ...
            if position > 0 and current_price < self._ma_value:
                return TradingSignal(
                    direction=SignalDirection.FLAT, price=current_price, reason="trend_reversal"
                )
