...
def _enqueue_rows(
    ...
    *,
    ...
    max_local_id = max(int(row.get("id", 0)) for row in rows)
    payload: dict[str, Any] = {payload_key: rows}
    if include_cursor:
        payload["_cursor"] = {"kind": kind, "max_local_id": max_local_id}
    if outbox.enqueue(kind, payload, batch_id=batch_id):
        return max(current_cursor, max_local_id), True
    logger.warning("Bridge enqueue skipped for kind=%s batch_id=%s; cursor remains at %s", kind, batch_id, current_cursor)
    return current_cursor, False
...
def rebuild_outbox_from_observability(
    ...
    *,
    ...
    if manifest:
        if outbox.enqueue("run_manifest", manifest, batch_id=f"replay_run_manifest_{run_id}"):
    ...
    while not _bridge_stop.is_set():
        try:
            ...
            if not sent_run_manifest and run_id and run_id != "unknown":
                ...
                if manifest:
                    if outbox.enqueue("run_manifest", manifest, batch_id=f"run_manifest_{run_id}"):
            ...
            while True:
                ...
                if not rows:
                    break
                next_cursor, queued = _enqueue_rows(
                    outbox,
                    "state_snapshots",
                    rows,
                    payload_key="state_snapshots",
                    batch_id=f"state_snapshots_{run_id}_{max(int(row.get('id', 0)) for row in rows)}",
                    current_cursor=last_state_id_sent,
                )
                if queued:
                    last_state_id_sent = next_cursor
                else:
                    break
                if len(rows) < 250:
                    break
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    "market_tape",
                    rows,
                    payload_key="market_tape",
                    batch_id=f"market_tape_{run_id}_{max(int(row.get('id', 0)) for row in rows)}",
                    current_cursor=last_market_id_sent,
                )
                if queued:
                    last_market_id_sent = next_cursor
                else:
                    break
                if len(rows) < 500:
                    break
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    "decision_snapshots",
                    rows,
                    payload_key="decision_snapshots",
                    batch_id=f"decision_snapshots_{run_id}_{max(int(row.get('id', 0)) for row in rows)}",
                    current_cursor=last_decision_id_sent,
                )
                if queued:
                    last_decision_id_sent = next_cursor
                else:
                    break
                if len(rows) < 250:
                    break
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    "order_lifecycle",
                    rows,
                    payload_key="order_lifecycle",
                    batch_id=f"order_lifecycle_{run_id}_{max(int(row.get('id', 0)) for row in rows)}",
                    current_cursor=last_order_id_sent,
                )
                if queued:
                    last_order_id_sent = next_cursor
                else:
                    break
                if len(rows) < 250:
                    break
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    events,
                    payload_key="events",
                    batch_id=f"events_{run_id}_{max(int(event.get('id', 0)) for event in events)}",
                    current_cursor=last_event_id_sent,
                )
                if queued:
                    last_event_id_sent = next_cursor
                else:
                    break
                if len(events) < 250:
                    break
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    trades,
                    payload_key="trades",
                    batch_id=f"trades_{run_id}_{max(int(trade.get('id', 0)) for trade in trades)}",
                    current_cursor=last_trade_id_sent,
                )
                if queued:
                    last_trade_id_sent = next_cursor
                else:
                    break
                if len(trades) < 250:
                    break
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    "account_trades",
                    account_trades,
                    payload_key="account_trades",
                    batch_id=f"account_trades_{run_id}_{max(int(trade.get('id', 0)) for trade in account_trades)}",
                    current_cursor=last_account_trade_id_sent,
                    include_cursor=True,
                )
                if queued:
                    last_account_trade_id_sent = next_cursor
                else:
                    break
            ...
            if health_rows:
                next_cursor, queued = _enqueue_rows(
                    ...
                    "bridge_health",
                    health_rows,
                    payload_key="bridge_health",
                    batch_id=f"bridge_health_{run_id}_{max(int(row.get('id', 0)) for row in health_rows)}",
                    current_cursor=last_state_health_id_sent,
                    include_cursor=True,
                )
                if queued:
                    last_state_health_id_sent = next_cursor
            ...
            while True:
                ...
                next_cursor, queued = _enqueue_rows(
                    ...
                    "runtime_logs",
                    rows,
                    payload_key="runtime_logs",
                    batch_id=f"runtime_logs_{run_id}_{max(int(row.get('id', 0)) for row in rows)}",
                    current_cursor=last_runtime_log_id_sent,
                    include_cursor=True,
                )
                if queued:
                    last_runtime_log_id_sent = next_cursor
                else:
                    break
                if len(rows) < 250:
                    break
