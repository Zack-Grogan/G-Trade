...
class TradingEngine:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self._position_entry_zone: Optional[str] = None
        ...
        self._broker_truth_snapshot: Dict[str, Any] = {}
    ...
    def reset_runtime_state(self, clear_history: bool = True) -> None:
        ...
        with self._lock:
            ...
            self._broker_truth_snapshot = {}
            self._last_broker_truth_refresh_at = 0.0
            self._last_broker_truth_contradictions = {}
    ...
    def _clear_pending_entry_metadata(self) -> None:
        self._pending_decision_id = None
        self._pending_attempt_id = None
        self._pending_position_id = None
        self._pending_trade_id = None
    ...
    def _broker_position_from_snapshot(self, positions: Optional[Dict[str, Any]]) -> int:
        ...
        for contract_id, position in positions.items():
            if self._snapshot_matches_selected_symbol(contract_id, position):
    ...
    def _snapshot_matches_selected_symbol(self, contract_id: Any, position: Any) -> bool:
        requested = self.symbol.upper()
        resolved_contract_id = None
        if not self._mock_mode:
            try:
                resolved_contract_id = self.client._resolve_contract_id(self.symbol)
            except Exception:
                resolved_contract_id = None

        for candidate in (contract_id, getattr(position, "symbol", "")):
            normalized = str(candidate or "").strip().upper()
            if not normalized:
                continue
            if normalized.startswith("/"):
                normalized = normalized[1:]
            if resolved_contract_id and normalized == str(resolved_contract_id).upper():
                return True
            if normalized == requested:
                return True
            if re.fullmatch(rf"{re.escape(requested)}[FGHJKMNQUVXZ]\d{{1,2}}", normalized):
                return True
    ...
    def _refresh_broker_truth_snapshot(
        self,
        *,
        ...
        if contradictions and contradictions != self._last_broker_truth_contradictions and any(bool(v) for v in contradictions.values()):
            self._record_event(
                ...
                reason="broker_truth_contradiction",
            )
        self._last_broker_truth_contradictions = contradictions
        return bundle

    def _adopt_broker_open_orders(
        self,
        ...
        *,
        ...
        reconcile = self.executor.reconcile_broker_open_orders(
            self.symbol,
            broker_orders,
            event_time=event_time,
            reason=reason,
        ...
        self._unresolved_entry_side = side
        self._unresolved_entry_submission_count = max(1, len(broker_order_ids))
        self._unresolved_entry_first_submitted_at = event_time
        self._unresolved_entry_last_submitted_at = event_time
        self._adopted_broker_orders_at_startup = adoption_source == "broker_startup_orders"
        self._adoption_source = adoption_source
        ...
        self._record_event(
            ...
            payload={
                ...
                "executor_reconciliation": reconcile,
            },
            event_time=event_time,
            action="broker_order_adopt",
            reason=reason,
        )
    ...
    def _adopt_broker_state_at_startup(self) -> None:
        ...
        with self._lock:
            ...
            if broker_position == 0 and broker_order_ids:
                self._adopt_broker_open_orders(
                    broker_orders or [],
                    event_time=event_time,
                    adoption_source="broker_startup_orders",
                    reason="startup_adopt_orders",
                )
    ...
    def _evaluate_current_state(self, allow_entries: bool) -> None:
        ...
        if not zone_live_entries_allowed:
            self._last_entry_block_reason = zone_gate_reason
            self._record_decision_event(
                decision,
                zone=zone,
                current_time=current_time,
    ...
    def _sync_position_state(self) -> None:
        if self._mock_mode:
            ...
            positions, position_error = self.client.get_positions_snapshot()
            broker_orders, order_error = self.client.get_open_orders_snapshot(self.symbol)
            signed_position = self._broker_position_from_snapshot(positions)
            entry_price = self._broker_entry_price_from_snapshot(positions) or 0.0
            position_authoritative = position_error is None
        ...
        with self._lock:
            ...
            if not self._mock_mode and order_error is None:
                broker_order_reconciliation = self.executor.reconcile_broker_open_orders(
                    ...
                    broker_orders or [],
                    event_time=event_time,
                    reason="broker_position_sync",
                    broker_position=signed_position,
                )
                broker_order_ids = broker_order_reconciliation["broker_order_ids"]

            if not self._mock_mode and signed_position == 0 and broker_order_ids:
                self._adopt_broker_open_orders(
                    broker_orders or [],
                    event_time=event_time,
                    adoption_source="broker_sync_orders",
                ...
                if order_error is None and (broker_orders is None or len(broker_orders) == 0):
                    ...
                    self._record_event(
                        ...
                        payload={
                            ...
                            "signed_position": 0,
                            "cleared_order_ids": broker_order_reconciliation["cleared_order_ids"],
                            "adopted_order_ids": broker_order_reconciliation["adopted_order_ids"],
        ...
        execution_snapshot.update(
            {
                ...
                "live_entry_zones": list(self.config.strategy.live_entry_zones or []),
                "shadow_entry_zones": list(self.config.strategy.shadow_entry_zones or []),
                "desync_flags": {
