...
def _warn_deprecated_config_key(cls_name: str, key: str, detail: str) -> None:
    marker = (cls_name, key)
    if marker in _WARNED_DEPRECATED_CONFIG_KEYS:
        return
    _WARNED_DEPRECATED_CONFIG_KEYS.add(marker)
    warnings.warn(
        f"Deprecated config key '{key}' for {cls_name} is ignored: {detail}",
        UserWarning,
        stacklevel=3,
    )
...
def _dict_to_dataclass(d: Dict[str, Any], cls: type) -> Any:
    ...
    deprecated_keys = _DEPRECATED_CONFIG_KEYS.get(cls, {})
    kwargs = {}
    for key, value in d.items():
        if key in deprecated_keys:
            _warn_deprecated_config_key(cls.__name__, key, deprecated_keys[key])
            continue
