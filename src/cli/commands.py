...
def _fetch_broker_truth(
    cfg,
    *,
    symbol: str = "ES",
    lookback_minutes: int = 240,
    focus_timestamp: Optional[str] = None,
) -> dict[str, Any]:
    _ = cfg
    ...
    return client.get_broker_truth_bundle(
        symbol,
...
def _default_research_output(name: str) -> Path:
...
def _resolve_analysis_account_id() -> Optional[str]:
    state = get_state()
    if state.account_id:
...
@cli.group()
def analyze():
    """Local research, trade review, and launch-readiness checks."""
...
def analyze_trade_review(trade_key: str, account_id: Optional[str]) -> None:
    ...
    effective_account = account_id or _resolve_analysis_account_id()
    _print_json(build_trade_review(trade_key, account_id=effective_account))
