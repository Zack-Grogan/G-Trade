...
**Deploy preference:** Prefer pushing to the service's GitHub repo; use Railway MCP when available. Use CLI only when MCP isn't working or the operation isn't in MCP. When using CLI for a deploy: do not push to GitHub in the same iteration until recovery is verified (status SUCCESS, logs OK). After the fix is confirmed, push once and document (Linear, OPERATOR/Current-State/tasks). See rule 71-railway-iteration-single-build-path.
