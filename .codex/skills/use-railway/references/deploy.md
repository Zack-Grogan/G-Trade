...
## Deploy preference and iteration

Prefer deploying by pushing to the service's GitHub repo. Use Railway MCP when available. Use the CLI only when MCP isn't working or the operation isn't in MCP.

When you do use the CLI for a deploy: use **one** build path per iteration (GitHub push and `railway up` both trigger builds). From the repo: `railway up --service <service> -m "<summary>"`. Verify with `railway logs` and `railway service status --all --json`. Do not push to GitHub in the same iteration until the fix is confirmed. After verification, push once and document (Linear, docs). See rule 71-railway-iteration-single-build-path.
