...
1. Prefer deploying by pushing to the service's GitHub repo. Use Railway MCP for status, logs, and operations when available. Use Railway CLI only when MCP isn't working or the resource/operation isn't natively available in MCP. Fall back to `scripts/railway-api.sh` for operations the CLI doesn't expose.
...
For complex Railway operations, dispatch specialized subagents (max 2 at a time):

| When | Subagent | Use for |
|------|----------|---------|
| Complex infrastructure changes | **devops-automator** | Multi-service deployments, CI/CD pipeline changes, infrastructure automation. |
| Reliability / SRE concerns | **sre-site-reliability-engineer** | SLOs, error budgets, observability setup, reliability improvements. |
| Incident response | **incident-response-commander** | Production incidents, coordinated response, post-mortems. |
| Shell commands | **shell** | Running railway CLI commands, scripts, or any terminal execution. |

Dispatch patterns:
- **Deploy + verify:** shell (deploy) → shell (verify status/logs)
- **Incident:** incident-response-commander (coordinate) → shell (execute fixes)
- **Infrastructure change:** devops-automator (plan) → shell (execute)
...
## Deploy preference and single build path

Prefer deploying by pushing to the service's GitHub repo. Use Railway MCP for status, logs, and operations when available. Use Railway CLI only when MCP isn't working or the operation isn't natively available.

When you do use the CLI for a deploy: use **one** build path per iteration. GitHub push and `railway up` both trigger builds; don't do both in the same session. From the service's repo/directory: `railway up --service <service> -m "<summary>"` (or `--ci` to stream logs). Verify with `railway logs --service <service>` and `railway service status --all --json`. After the fix is confirmed, push to GitHub once and document (Linear, OPERATOR.md, etc.). See [deploy.md](references/deploy.md) and [operate.md](references/operate.md); rule: `.cursor/rules/71-railway-iteration-single-build-path.mdc`.
