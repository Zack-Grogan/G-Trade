...
For complex Upstash integration work, dispatch specialized subagents (max 2 at a time):

| When | Subagent | Use for |
|------|----------|---------|
| Backend integration | **backend-architect** | API design, service architecture, integration patterns. |
| Database optimization | **database-optimizer** | Schema design, query optimization, indexing, performance tuning. |
| Performance analysis | **performance-benchmarker** | Measuring latency, throughput, benchmarking. |

Max 2 subagents at a time. For complex integration work, run sequentially (e.g. backend-architect → database-optimizer).
