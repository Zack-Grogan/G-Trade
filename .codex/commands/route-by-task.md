...
2. Identify which task type matches the current request (issue-driven work, local trader/runtime, docs, safety, architecture, exploratory, templates).
3. **Subagent dispatch:** Check the agent-index **Subagent(s)** and **Pattern** columns. If the concern is route-first or hybrid and a subagent is listed, consider dispatching via `mcp_task`. Examples: broad codebase search → **explore** (set thoroughness); PR or diff review → **code-reviewer**; status/progress/analysis → **explore** and/or **technical-writer** (read `AGENTS.md`, `docs/Current-State.md`, `docs/README.md` first); API testing → **api-tester**; shell commands → **shell**. See AGENTS.md "Subagent routing" and rule 03-orchestration-first.
