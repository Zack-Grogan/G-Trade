...
class BrokerTruthBundleTests(unittest.TestCase):
    def setUp(self) -> None:
        self.config = Config()
        set_config(self.config)

    def _make_client(self) -> TopstepClient:
        client = TopstepClient(self.config.api)
        client._access_token = "token"
        client._token_expires = time.time() + 3600
        client._account_id = 20139389
        client._account = client._account_summary(
            {
                "id": "20139389",
    ...
    def test_broker_truth_bundle_does_not_match_mes_contracts_to_es(self) -> None:
        client = self._make_client()
        focus_timestamp = "2026-03-19T11:35:01Z"

        with patch.object(client, "get_positions_snapshot", return_value=({}, None)):
            with patch.object(client, "get_open_orders_snapshot", return_value=([], None)):
                with patch.object(client, "_resolve_contract_id", return_value="CON.F.US.EP.M26"):
                    with patch.object(
                        client,
                        "search_orders",
                        return_value=[
                            {
                                ...
                                "symbol": "MESM26",
                                "status": "working",
                                "creationTimestamp": focus_timestamp,
                            }
                        ],
                    ):
                        with patch.object(
    ...
    def test_broker_truth_bundle_limits_recent_activity_contradiction_to_focus_window(self) -> None:
        client = self._make_client()
        focus_dt = datetime(2026, 3, 19, 11, 35, 1, tzinfo=pytz.UTC)
        old_timestamp = (focus_dt - timedelta(minutes=30)).isoformat()

        with patch.object(client, "get_positions_snapshot", return_value=({}, None)):
            with patch.object(client, "get_open_orders_snapshot", return_value=([], None)):
                with patch.object(client, "_resolve_contract_id", return_value=None):
                    with patch.object(
                        client,
                        "search_orders",
                        return_value=[
    ...
    def test_get_positions_snapshot_treats_type_2_as_short(self) -> None:
        client = self._make_client()
        ...
        with patch.object(client, "_post_with_retry", return_value=_Response()):
            positions, error = client.get_positions_snapshot()

        self.assertIsNone(error)
        assert positions is not None
        self.assertEqual(positions["CON.F.US.EP.M26"].quantity, -1)
