...
- **Railway:** Set `DATABASE_URL` (from Postgres), `INGEST_API_KEY`, `ANALYTICS_API_KEY`, `X_AI_API_KEY`, `QSTASH_TOKEN`, and for RLM `RLM_REPLAY_WORKER_URL` = RLM service public URL + `/replay/worker`.
...
- **RLM:** Root = `railway/rlm`, start = `uvicorn app:app --host 0.0.0.0 --port $PORT` (or use `Procfile`). Env: `DATABASE_URL`, `X_AI_API_KEY`, `QSTASH_TOKEN`, `RLM_REPLAY_WORKER_URL` (RLM public URL + `/replay/worker`). Optional: `UPSTASH_REDIS_*`, `UPSTASH_VECTOR_*`, `DAYTONA_API_KEY`.
...
3. **Env per service** (set in Railway dashboard or CLI):
   ...
   - **rlm:** `DATABASE_URL`, `X_AI_API_KEY`, `QSTASH_TOKEN`, `RLM_REPLAY_WORKER_URL` (RLM public URL + `/replay/worker`). Optional: `UPSTASH_REDIS_*`, `UPSTASH_VECTOR_*`, `DAYTONA_API_KEY`
