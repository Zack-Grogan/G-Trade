...
from src.bridge.railway_bridge import _enqueue_rows, _record_delivery_success, rebuild_outbox_from_observability, start_railway_bridge, stop_railway_bridge
...
class TestRailwayBridgeDeliveryState(unittest.TestCase):
    ...
    def test_enqueue_rows_does_not_advance_cursor_on_enqueue_failure(self) -> None:
        class _FakeOutbox:
            def enqueue(self, kind: str, payload: dict, *, batch_id: str) -> bool:
                return False
        ...
        next_cursor, queued = _enqueue_rows(
            ...
            [{"id": 41}, {"id": 42}],
            payload_key="events",
            batch_id="events_batch",
            current_cursor=40,
        )

        self.assertFalse(queued)
        self.assertEqual(next_cursor, 40)

    def test_rebuild_outbox_only_counts_manifest_when_enqueue_succeeds(self) -> None:
        class _FakeStore:
            ...
            def query_completed_trades(self, **kwargs):
                return []

            def query_account_trades(self, **kwargs):
                return []

            def query_state_snapshots(self, **kwargs):
                return []

            def query_market_tape(self, **kwargs):
                return []

            def query_decision_snapshots(self, **kwargs):
                return []

            def query_order_lifecycle(self, **kwargs):
                return []

            def query_bridge_health(self, **kwargs):
                return []

            def query_runtime_logs(self, **kwargs):
                return []
        ...
        class _FakeOutbox:
            def get_delivery_state(self) -> dict:
                return {}

            def enqueue(self, kind: str, payload: dict, *, batch_id: str) -> bool:
                return False

        with patch("src.bridge.railway_bridge.get_observability_store", return_value=_FakeStore()):
            counts = rebuild_outbox_from_observability(_FakeOutbox(), run_id="r1")
